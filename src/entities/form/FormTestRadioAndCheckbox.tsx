...
const initialValues: IFormTestRadioAndCheckbox = {
	checkbox: [],
...
const validationSchema = Yup.object({
	checkbox: Yup.array()
		.min(1, "Необходимо выбрать вариант")
...
function FormTestRadioAndCheckbox() {
	...
	return (
		<Formik
			...
			validateOnBlur={true}
			onSubmit={(values: IFormTestRadioAndCheckbox) => {
				console.log(values);
		...
		>
			<Form className={"flex flex-col gap-4 items-center"}>
				<CustomField
					name={"checkbox"}
					as={Checkbox}
					label={"Готовы приступить к работе?"}
				/>
				<CustomField
					name={"radiogroup"}
					as={RadioGroup}
					label={"Взять на работу?"}
					options={["Да", "Конечно, да"]}
				/>
				<Button
					type={"submit"}
					className={
